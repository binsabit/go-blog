...
func (r *userR) GetBlogs() BlogSlice {
	if r == nil {
		return nil
	}
	return r.Blogs
...
func (userL) LoadBlogs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`blogs`),
		qm.WhereIn(`blogs.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetBlogs removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Blogs accordingly.
// Replaces o.R.Blogs with related.
// Sets related.R.User's Blogs accordingly.
func (o *User) SetBlogs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Blog) error {
	query := "update \"blogs\" set \"user_id\" = null where \"user_id\" = $1"
	...
	return o.AddBlogs(ctx, exec, insert, related...)
}

// RemoveBlogs relationships from objects passed in.
// Removes related items from R.Blogs (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveBlogs(ctx context.Context, exec boil.ContextExecutor, related ...*Blog) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Blogs {
			...
			ln := len(o.R.Blogs)
			if ln > 1 && i < ln-1 {
				o.R.Blogs[i] = o.R.Blogs[ln-1]
			}
			o.R.Blogs = o.R.Blogs[:ln-1]
			break
		}
	}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Comments accordingly.
// Replaces o.R.Comments with related.
// Sets related.R.User's Comments accordingly.
func (o *User) SetComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	query := "update \"comments\" set \"user_id\" = null where \"user_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
// Sets related.R.User.
func (o *User) RemoveComments(ctx context.Context, exec boil.ContextExecutor, related ...*Comment) error {
	if len(related) == 0 {
		return nil
	}
